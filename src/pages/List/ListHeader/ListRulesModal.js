...
function BFRules({ list }) {
  ...
  const article = ["A", "E", "I", "O", "U"].includes(list.battleForce[0])
    ? "an"
    : "a";

  if (!bfRules) {
    return (
      <Typography style={{ marginBottom: 0 }} gutterBottom>
        See your rule doc for full battleforce rules while playing.
      </Typography>
    );
  }
  ...
  return (
    <div>
      <Typography style={{ marginBottom: 0 }} variant="subtitle1" gutterBottom>
        Special Rules:
      </Typography>
      <DialogContent>
        {bfRules.noFieldComm && (
          <Typography className={classes.bfRules}>
            The Field Commander keyword cannot be used in {article}{" "}
            {list.battleForce} army.
          </Typography>
        )}
        {bfRules.countMercs && (
          <div>
            {list.battleForce !== "Shadow Collective" && (
              <Typography className={classes.bfRules}>
                This Battle Force may take any number of Mercenary units.
              </Typography>
            )}
            {list.battleForce === "Shadow Collective" && (
              <Typography className={classes.bfRules}>
                This Battle Force must be composed entirely of Mercenary units.
              </Typography>
            )}
            <Typography className={classes.bfRules}>
              These units do count towards the miniumum number of Ranks required
              and the army may include any number of Mercenaries at each Rank.
            </Typography>
          </div>
        )}
        {bfRules.take2NonEwokRebs && (
          <Typography className={classes.bfRules}>
            This Battle Force must take at least 2 non-EWOK, REBEL units.
          </Typography>
        )}
        {bfRules.buildsAsCorps && (
          <Typography className={classes.bfRules}>
            The following unit{bfRules.buildsAsCorps.length > 1 ? "s" : ""}{" "}
            counts as a CORPS unit for the purposes of army building
            {bfRules.ccAsCorps ? " and Battle Cards" : ""}:{" "}
            {bfRules.buildsAsCorps
              .map((id) =>
        ...
        {bfRules.minOneOfEachCorps && (
          <Typography className={classes.bfRules}>
            When building an army, {article} {list.battleForce} Battle Force
            cannot take more than 1 of each with a CORPS rank until at least one
            of each has been taken. This army may not include detachments.
          </Typography>
        )}
        {bfRules.remnantEquipRules && (
          <Typography className={classes.bfRules}>
            Any non-droid trooper unit in this army with a HEAVY WEAPON upgrade
            icon may equip a HEAVY WEAPON upgrade with one of the following unit
            requirements, ignoring that requirement: Stormtroopers only,
            Shoretroopers only, Scout Troopers only, or Imperial Death Troopers
            only.
          </Typography>
        )}
        {bfRules.ignoreDetach && (
          <Typography className={classes.bfRules}>
            {cards[bfRules.ignoreDetach].cardName} units in this army do not
            have the DETACHMENT keyword.
          </Typography>
        )}
        {battleForcesDict[list.battleForce].plainTextRules?.map((r) => (
          <Typography className={classes.bfRules}>{r}</Typography>
        ))}
      </DialogContent>
        ...
        if (r === "commOp") {
          return (
            <div className={classes.row} style={{ marginBottom: 5 }}>
              ...
              />
              <Typography
                style={{ fontSize: 20, marginLeft: 2, marginRight: 2 }}
              >
                /
              </Typography>
              <Avatar
              ...
              <DialogContentText style={{ marginBottom: 0 }}>
                1 - {rankReqs.commOp} (minimum 1 Commander)
              </DialogContentText>
            </div>
          );
        }
        ...
        if (rankReqs[r][1] > 0) {
          ...
          if (bf) {
            ...
            unitLinks = bf[r].map((id, idx) => {
              ...
              let name = cards[id].displayName
                ? cards[id].displayName
                : cards[id].cardName;
              let unitLimit = bf.rules.unitLimits?.find((l) =>
                l.ids.includes(id),
              );
              let limit = "";
              ...
              if (unitLimit) {
                ...
                if (unitLimit.ids.length > 1) {
                  let allNames = unitLimit.ids.map((id) =>
                    cards[id].displayName
                      ? cards[id].displayName
                      : cards[id].cardName,
                  );
          ...
          return (
            <div className={classes.row} style={{ marginBottom: 5 }}>
              ...
              />
              <DialogContentText style={{ marginBottom: 0, minWidth: 40 }}>
                {rankReqs[r][0]} - {rankReqs[r][1]}
              </DialogContentText>
              <div
                style={{
                  display: "flex",
...
function ListRulesModal({ open, handleClose }) {
  ...
  const { currentList, handleSetBattleForce } = useContext(ListContext);

  const bfName = currentList.battleForce
    ? currentList.battleForce
    : currentList.faction.charAt(0).toUpperCase() +
      currentList.faction.slice(1) +
      " (No Battleforce)";

  const rankLabel = currentList.battleForce
    ? "Rank and Unit Requirements:"
    : "Rank Requirements:";
