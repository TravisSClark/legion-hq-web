...
function countPoints(list) {
  ...
  list.units.forEach((unit, unitIndex) => {
    ...
    if (list.isUsingOldPoints) {
      unit.totalUnitCost = unitCard.oldCost ? unitCard.oldCost : unitCard.cost;
    ...
    unit.upgradesEquipped.forEach((upgradeId, upgradeIndex) => {
      if (upgradeId) {
        ...
        if (list.isUsingOldPoints) {
          unit.totalUnitCost += upgradeCard.oldCost ? upgradeCard.oldCost : upgradeCard.cost;
    ...
    if (unit.counterpart) {
      ...
      if (list.isUsingOldPoints) {
        unit.counterpart.totalUnitCost = counterpartCard.oldCost ? counterpartCard.oldCost : counterpartCard.cost;
      ...
      unit.counterpart.upgradesEquipped.forEach(upgradeId => {
        if (upgradeId) {
          const upgradeCard = cards[upgradeId];
          if (list.isUsingOldPoints) {
            unit.counterpart.totalUnitCost += upgradeCard.oldCost ? upgradeCard.oldCost : upgradeCard.cost;
...
function toggleCardsForOldMode(list){
  list.pointTotal = 0;
  list.units.forEach((unit, unitIndex) => {
    const unitCard = cards[unit.unitId];
    if (list.isUsingOldPoints) {
      if(unitCard.oldCard){
        unit.unitId = unitCard.oldCard;
      }
    } else {
      if(unitCard.newCard){
        unit.unitId = unitCard.newCard;
      }
    }
    ...
    unit.upgradesEquipped.forEach((upgradeId, upgradeIndex) => {
      if (upgradeId) {
        ...
        let toggleUpgradeId = "";
        if (list.isUsingOldPoints) {
          if(upgradeCard.oldCard){
            toggleUpgradeId = upgradeCard.oldCard;
          }
        } else {
          if(upgradeCard.newCard){
            toggleUpgradeId = upgradeCard.newCard;
          }
        }
        if(toggleUpgradeId){
          unit.upgradesEquipped.splice(upgradeIndex, 1, toggleUpgradeId);
        }
      }
    });
    if (unit.counterpart) {
      const counterpartCard = cards[unit.counterpart.counterpartId];
        let toggleUpgradeId = "";
        if (list.isUsingOldPoints) {
          if(counterpartCard.oldCard){
            toggleUpgradeId = counterpartCard.oldCard;
          }
        } else {
          if(counterpartCard.newCard){
            toggleUpgradeId = counterpartCard.newCard;
          }
        }
        if(toggleUpgradeId){
          unit.counterpart.counterpartId = toggleUpgradeId;
        }
    }
  });
...
function toggleUsingOldPoints(list) {
  ...
  list = toggleCardsForOldMode(list);
...
function consolidate(list) {
  ...
  for (let i = 0; i < list.units.length; i++) {
    ...
    if (unitCard.detachment){
      let parent = list.units.find(u => u.unitId == unitCard.detachment)
      if(parent != undefined){
        list.unitCounts[unitCard.rank] -= Math.min(unit.count, parent.count);
      }
    }
...
function getEligibleUnitsToAdd(list, rank, userSettings) {
  ...
  for (let i = 0; i < cardsById.length; i++) {
    ...
    if((id === 'AA' || id === 'AK') && !userSettings.showStormTide)
      continue;
...
// This doesn't feel great as a spot for most of these, but it's not horrible re how validation works now
...
function validateList(currentList, rankLimits){
  ...
  let ranks = {...currentList.unitCounts}
...
function checkBattleforceUnits(list, battleforceName){
  if(battleforceName == ''){
    // TODO special issue
    return [];
  }
  const battleforce = battleForcesDict[battleforceName];
  let invalidUnits = [];

  console.log(JSON.stringify(list));
  console.log(JSON.stringify(battleforce));

  list.units.forEach(unit => {
    let card = cards[unit.unitId];
    console.log(JSON.stringify(card));
    if(!battleforce[card.rank].includes(unit.unitId)){
      console.log("Illegal unit! " + JSON.stringify(unit));
      invalidUnits.push(unit);
      
    }
    else if(unit.hasUniques){
      // iterate thru upgrades, find uniques, flag unit if has unique upgrade not on bf whitelist
    }
