...
function expandKeywords(obj, array, allTheCardsCsv, allTheCardsJson = null) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    switch (expandedKeywords[expandedKeywords.length - 1].name) {
      case "Associate":
        ids = allTheCardsCsv
          .filter((c) => c[indices.NAME] == newKeyword.value)
          .map((c) => c[indices.ID]);
        obj.associate = ids[0];
        break;
      case "Detachment":
        ...
        ids = allTheCardsCsv
          .filter(
            (c) =>
              c[indices.NAME] == newKeyword.value &&
              c[indices.FACTION] == obj.faction &&
              c[indices.KEYWORDS].indexOf("Detachment") == -1
          )
          .map((c) => c[indices.ID]);
      ...
      case "Mercenary":
        obj.affiliations = newKeyword.value
          .split(",")
        ...
        if (obj.affiliations[0].toLowerCase() == "all") {
          ...
          while (
            i + 1 < array.length &&
            factions.includes(array[i + 1].toLowerCase())
          ) {
      ...
      case "Equip":
        ...
        equips.forEach((e) => {
          // friggin' Bad Batch...
          ids = Object.keys(allTheCardsJson).filter(
            (c) =>
              (e != "Hunter" || allTheCardsJson[c].isUnique) &&
              allTheCardsJson[c].cardType == "upgrade" &&
              (
                allTheCardsJson[c].cardName +
                (allTheCardsJson[c].title ? allTheCardsJson.title : "")
...
function convertUnit(l, cleanedArray, cardData) {
  ...
  for (let i = indices.WEAPON1; l.length > i && l[i]; i += 7) {
    ...
    expandKeywords(
      newObj.weapons[newObj.weapons.length - 1],
      newObj.weapons[newObj.weapons.length - 1].keywords,
      cleanedArray
    );
  }
  ...
  if (cardData[id]) {
    ...
    if (ogCardHistory) {
      newObj.history = JSON.parse(JSON.stringify(ogCardHistory));
    }
  } else {
