...
function validateKeywords(c, keywords) {
  ...
  keywords.forEach((k) => {
    ...
    const kwNeedingField = keywordsWithFields.find((kw) => kw[0] === kwName);
    ...
    if (cwVal == undefined) {
      ...
      if (cwVal && k.value == undefined) {
        ...
        console.log(
          "found kw.value when not expecting one",
          c.cardName,
          kwName,
        );
      }
    ...
    const matchingKeyword = keywordNames.find((a) => a === k || a === k.name);
    if (!matchingKeyword) {
      console.log(
...
async function cardsJsonCheck() {
  ...
  console.warn(
    "'Aligned' Force upgrades are currently bugged for no-unit detection",
  );
  cardIdsByType.upgrade.forEach((id) => {
    ...
    for (let i = 0; !hasUnit && i < cardIdsByType.unit.length; i++) {
      hasUnit = areRequirementsMet(
        upgradeCard.requirements,
        cards[cardIdsByType.unit[i]],
      );
    }
    if (!hasUnit) {
