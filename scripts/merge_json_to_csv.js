...
function mergeUnits(unitArray) {
  ...
  cleanedArray.push(
    "##lhqid, cost, name, title, displayName, ttsName, imageName, upgrades,  isUnique, rank, minicount, faction, affiliation, unit type, def, wounds, courage, speed, hitsurge, defsurge, keywords"
  );
  for (let i = 1; i <= 3; i++) {
  ...
  for (let a = 1; a < linesArray.length; a++) {
    ...
    let isMerc = !["empire", "rebels", "republic", "separatists"].includes(
      line[7]
    );
    let faction = line[7];
    let affiliation = "";
...
function splitJson() {
  ...
  fs.writeFileSync(
    "counterpartCards.json",
    JSON.stringify(cardsByType.counterpart)
  );
...
function joinJson() {
  let battleCardsData = JSON.parse(
    fs.readFileSync("./battleCards.json", "utf8")
  );
  let commandCardsData = JSON.parse(
    fs.readFileSync("./commandCards.json", "utf8")
  );
  let counterpartCardsData = JSON.parse(
    fs.readFileSync("./counterpartCards.json", "utf8")
  );
  let unitCardsData = JSON.parse(fs.readFileSync("./unitCards.json", "utf8"));
  let upgradeCardsData = JSON.parse(
    fs.readFileSync("./upgradeCards.json", "utf8")
  );

  let fullData = {
    ...battleCardsData,
    ...commandCardsData,
    ...counterpartCardsData,
    ...unitCardsData,
    ...upgradeCardsData,
  };

  const orderedData = Object.keys(fullData)
    .sort()
    .reduce((obj, key) => {
